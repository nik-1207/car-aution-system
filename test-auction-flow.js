...
class TestRunner {
  ...
  async createAuction() {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/auction/createAuction`, auctionData, {
    ...
    for (let round = 1; round <= bidsPerDealer; round++) {
      ...
      for (let i = 0; i < dealerTokens.length; i++) {
        try {
          ...
          const response = await axios.post(`${BASE_URL}/auction/placeBids`, bidData, {
          ...
          console.error(
            `❌ Bid failed for ${dealerTokens[i].dealer.name}:`,
  ...
  async fetchAuctionWinner() {
    ...
    try {
      const response = await axios.get(`${BASE_URL}/auction/${auctionId}/winner-bid`, {
        headers: { Authorization: `Bearer ${adminToken}` },
      });
